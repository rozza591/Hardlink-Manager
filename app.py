...
from collections import defaultdict  # For grouping files by size/hash
import json  # For saving/loading results
from flask import Flask, request, jsonify, render_template, send_from_directory, Response, send_file  # Web framework and utilities
import sys
import multiprocessing  # For running scans/links in separate processes
from concurrent.futures import ProcessPoolExecutor  # For parallel hashing
import stat  # For checking file types (e.g., symbolic links)
import io # For handling PDF generation in memory
...
def format_bytes(bytes_val, decimals=2):
    ...
    if not isinstance(bytes_val, (int, float)) or bytes_val < 0 or not bytes_val: return '0 Bytes'
    k=1024 # Kilobyte base
    dm=decimals if decimals >= 0 else 0 # Number of decimal places
    sizes=['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'] # Size units
    if bytes_val < 1: return '0 Bytes'

    # Calculate the appropriate size index
    power = int(bytes_val.bit_length() / 10.0) # Approximate power of 1024
    i = min(int(power / 10 * 3), len(sizes) - 1)

    # Handle edge case where small values might initially be classified as Bytes but are >= 1KB
    if bytes_val > 0 and i == 0 and bytes_val >= k:
         i = 1

    denominator = k**i
    if denominator == 0: denominator = 1 # Avoid division by zero
    return f"{bytes_val / denominator:.{dm}f} {sizes[i]}"
...
def run_manual_scan_and_link(scan_id, path1, dry_run, link_type, save_automatically, progress_info_managed, scan_results_managed):
    ...
    Args:
        scan_id (str): Unique identifier for this scan operation.
        path1 (str): The directory path to scan.
        dry_run (bool): If True, only report duplicates; don't link.
        link_type (str | None): 'hard' or 'soft' if linking is requested, else None.
        save_automatically (bool): If True, save JSON/PDF results to the scan directory on success.
        progress_info_managed (multiprocessing.Manager.dict): Shared dict for progress updates.
        scan_results_managed (multiprocessing.Manager.dict): Shared dict to store final results.
    """
    ...
    try:
        ...
        for entry in os.scandir(path1):
            if entry.is_dir(follow_symlinks=False):
                 ...
                 for dirpath, _, filenames in os.walk(entry.path):
                      ...
                      relative_dir = os.path.relpath(dirpath, path1); status_update = f"Scanning: .{os.sep}{relative_dir}"
                      if total_files_found % 500 == 0: update_progress(progress_info_managed, scan_id, {"status": status_update, "processed_items": total_files_found})
        ...
        files_by_hash = defaultdict(list) # {(size, hash): [{'path': ..., 'inode': ..., 'hash': ...}, ...]} <- MODIFIED STRUCTURE
        ...
        hashed_file_count = 0
        update_progress(progress_info_managed, scan_id, {"phase": "Hashing Files", "status": f"Preparing to hash {potential_dupe_file_count} potentials...", "total_items": potential_dupe_file_count, "processed_items": 0})

        # Perform hashing only if potential duplicates were found
        if potential_dupe_file_count > 0:
            filepaths_to_hash = [info['path'] for info in files_to_hash_info]
            # Map file paths back to their full info (including inode) for later use
            info_map = {info['path']: info for info in files_to_hash_info}
            # Use a ProcessPoolExecutor for parallel hashing (adjust worker count as needed)
            num_workers = max(1, os.cpu_count() // 2 if os.cpu_count() else 1)
            logging.info(f"[Scan {scan_id}] Starting parallel hash with {num_workers} workers for {potential_dupe_file_count} files.")

            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                # Map the calculate_hash function over the list of filepaths
                hash_results = executor.map(calculate_hash, filepaths_to_hash)
                ...
                for filepath, filehash in hash_results:
                    ...
                    if hashed_file_count % 100 == 0 or hashed_file_count == potential_dupe_file_count:
                         percentage = round((hashed_file_count * 100) / potential_dupe_file_count) if potential_dupe_file_count else 100
                         update_progress(progress_info_managed, scan_id, {"status": f"Hashing {hashed_file_count}/{potential_dupe_file_count}", "processed_items": hashed_file_count, "percentage": percentage})
        ...
        if save_automatically and final_status == "done":
            ...
            # PDF generation removed as per previous request
...
def link_process_worker(link_op_id, scan_id, link_type, link_progress_managed, link_results_managed, scan_results_managed):
    ...
    (Verification still uses inodes as the primary method for hard links)

    Args:
        link_op_id (str): Unique ID for this specific linking operation.
        scan_id (str): The ID of the original dry run scan providing the duplicates list.
        link_type (str): 'hard' or 'soft'.
        link_progress_managed (multiprocessing.Manager.dict): Shared dict for this link op's progress.
        link_results_managed (multiprocessing.Manager.dict): Shared dict for this link op's final result.
        scan_results_managed (multiprocessing.Manager.dict): Shared dict holding original scan results.
    """
